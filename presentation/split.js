...
const anchors=['overview','findings','framework','evidence','recommendations','pilot-comparison','methodology','papers'];
...
function size(value){value=Math.max(30,Math.min(65,value));document.documentElement.style.setProperty('--left',value+'%');ratio.value=value;divider.setAttribute('aria-valuenow',Math.round(value));if(autoSync)locate()}
...
window.addEventListener('resize',()=>{if(autoSync)locate()});
...
window.RAVFinish=()=>document.querySelector('#exit').click();
